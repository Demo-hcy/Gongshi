...
from library.mqtt_lib.TSL_model.S7_FRT_RDM305_MODEL import S7_FRT_RDM305

mqtt_client = MQTTClient(host='192.168.49.96', port=1883)
platform = PlatformController(mqtt_client)
device = S7_FRT_RDM305('1700014')
# # 登录
# # platform.will_set()
# platform.start()
...
def service():
    service = device.services.setPicture
    service.parameters.picture.v=1
    r = platform.service_invoke(device, service)
    r = platform.read(device, [device.properties.picture])
    platform.stop()
